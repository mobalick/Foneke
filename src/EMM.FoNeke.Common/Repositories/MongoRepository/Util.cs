namespace MongoRepository
{
    ...
    internal static class Util<U>
    {
        ...
        private static string GetCollectionNameFromType(Type entitytype)
        {
            ...
            var att = Attribute.GetCustomAttribute(entitytype, typeof(CollectionName));
            string collectionname;
            if (att != null) collectionname = ((CollectionName)att).Name;
            else if (entitytype.GetGenericArguments().Length > 0)
                collectionname = entitytype.GetGenericArguments()[0].Name;
            else collectionname = entitytype.Name;
